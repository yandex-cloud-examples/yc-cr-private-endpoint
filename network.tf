...
resource "yandex_vpc_subnet" "nat_vm_subnets" {
  ...
  name           = "cr-subnet-${substr(var.yc_availability_zones[count.index], -1, -1)}"
  folder_id      = var.folder_id
...
resource "yandex_vpc_subnet" "test_subnet" {
  name           = "test-cr-subnet-${substr(var.yc_availability_zones[0], -1, -1)}"
  folder_id      = var.folder_id
...
// create internal DNS zone for Container Registry Endpoint
resource "yandex_dns_zone" "cr_zone" {
  folder_id = var.folder_id
  name        = "cr-zone"
  description = "Internal zone for cr Endpoint"
  zone             = "${var.cr_fqdn}."
  public           = false
  private_networks = [var.vpc_id == null ? yandex_vpc_network.vpc[0].id : var.vpc_id]
}

// create DNS 'A' record for Container Registry Endpoint mapped to IP address of internal NLB
resource "yandex_dns_recordset" "cr-endpoint-dns-rec" {
  zone_id = yandex_dns_zone.cr_zone.id
  name    = "@"
  type    = "A"
  ttl     = 300
  data    = ["${tolist(tolist(yandex_lb_network_load_balancer.cr_nlb.listener)[0].internal_address_spec)[0].address}"]  
}
